...
func (h *Handler) CreatePayment(c *gin.Context) {
	...
	if err != nil { response.Fail(c, http.StatusBadRequest, "Bad Request", "BAD_REQUEST", err.Error()); return }
...
func (h *Handler) GetPaymentStatus(c *gin.Context) {
	...
	if err != nil { response.Fail(c, http.StatusNotFound, "Not Found", "NOT_FOUND", err.Error()); return }
	response.OK(c, st)
}
...
func (h *Handler) handleWebhook(c *gin.Context, provider string) {
	...
	for k, v := range c.Request.Header { if len(v) > 0 { headers[k] = v[0] } }
	if err := h.svc.HandleWebhook(c.Request.Context(), provider, headers, payload); err != nil {
		response.Fail(c, http.StatusBadRequest, "Bad Request", "BAD_REQUEST", err.Error()); return
	}
