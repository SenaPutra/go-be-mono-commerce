...
func registerRoutes(v1 *gin.RouterGroup, cfg config.Config, db *gorm.DB) {
	...
	admin.POST("/products", ok); admin.PUT("/products/:id", ok); admin.DELETE("/products/:id", ok); admin.PATCH("/products/:id/publish", ok); admin.PATCH("/products/:id/unpublish", ok); admin.PUT("/products/:id/stock", ok)
	admin.POST("/categories", ok); admin.PUT("/categories/:id", ok); admin.DELETE("/categories/:id", ok)
	admin.GET("/orders", ok); admin.GET("/orders/:id", ok); admin.PATCH("/orders/:id/status", ok)
